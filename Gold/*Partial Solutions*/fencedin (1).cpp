...
vector<pair<pair<pii, pii>, int> > edges; // list of edges in the form pair<point1, point2>, length of wall between them
...
struct DSU {
	...
	pii get(pii x) { return p[x.first][x.second].first < 0 ? x : p[x.first][x.second] = get(p[x.first][x.second]); }
...
int main() {
	...
	for (int i = 0; i < m + 1; i++) {
		for (int j = 0; j < n; j++) { // add right-left neighbors' fences as edges
			edges.pb({{{i, j}, {i, j + 1}}, horizontalFences[i + 1] - horizontalFences[i]});
		}
	}
	for (int i = 0; i < n + 1; i++) {
		for (int j = 0; j < m; j++) { // add top-down neighbors' fences as edges
			edges.pb({{{j, i}, {j + 1, i}}, verticalFences[i + 1] - verticalFences[i]});
		}
	}
	cout << kruskals() << nl;
}
