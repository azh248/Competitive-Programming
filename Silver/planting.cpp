...
int main() {
	...
	for (int i = 0; i < 2 * n - 1; i++) {
		if (verticalEdges[i][1] == 0) {
			int lower = (*horizontalIndex.find({verticalEdges[i][3], verticalEdges[i][4] + 1})).second;
			int upper = (*horizontalIndex.find({verticalEdges[i][2], verticalEdges[i][4]})).second;
			for (int j = lower; j < upper; j++) {
				overlapCount[j]++;
			}
		} else {
			int lower = (*horizontalIndex.find({verticalEdges[i][3], verticalEdges[i][4]})).second;
			int upper = (*horizontalIndex.find({verticalEdges[i][2], verticalEdges[i][4] - 1})).second;
			for (int j = lower; j < upper; j++) {
				overlapCount[j]--;
			}
		}
